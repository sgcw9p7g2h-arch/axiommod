...
final class NbtReader {
    ...
    private Object readPayload(int type) throws IOException {
        return switch(type){
            ...
            case FLOAT -> in.readFloat(); case DOUBLE -> in.readDouble(); case BYTE_ARRAY -> { int n=in.readInt(); if(n<0) throw new IOException("negative byte array"); byte[] a=new byte[n]; in.readFully(a); yield a; }
            case STRING -> in.readUTF();
            case LIST -> { int child=in.readUnsignedByte(); int n=in.readInt(); if(n<0) throw new IOException("negative list"); List<Object> list=new ArrayList<>(n); for(int i=0;i<n;i++) list.add(readPayload(child)); yield new NbtList(child,list); }
            ...
            case INT_ARRAY -> { int n=in.readInt(); if(n<0) throw new IOException("negative int array"); int[] a=new int[n]; for(int i=0;i<n;i++) a[i]=in.readInt(); yield a; }
            case LONG_ARRAY -> { int n=in.readInt(); if(n<0) throw new IOException("negative long array"); long[] a=new long[n]; for(int i=0;i<n;i++) a[i]=in.readLong(); yield a; }
